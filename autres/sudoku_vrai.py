...
def recreer_menu(root):
    global menu, aide
    menu = tk.Menu(root)
    Accueil = menu.add_command(label="Accueil", command=lambda: (effacer_widget_sans_forget(root), creation_menu()))
    menu.add_separator()
    ...
    menu.add_separator()
    Sauvegarder = menu.add_command(label="Sauvegarder", command=lambda: ajouter_sauvegarde(sudoku_a_completer), state="disabled")
    menu.add_separator()

    Aide = tk.Menu(menu, tearoff=0)
    Aide.add_checkbutton(label="Activer l'aide", variable=aide, command=lambda: activer_aide(aide))
    Aide.add_command(label="Voir les régles en ligne", command=lambda:webbrowser.open("https://www.le-sudoku.fr/le-sudoku/regles"))
...
def creation_menu():
    ...
    button_nvl_partie = tk.Button(frame_buttons, text="Nouvelle partie", font=("Arial", 27), command=lambda: (effacer_widget_sans_forget(root), recreer_menu(root), nouvelle_partie()))
    button_nvl_partie.pack(pady=10)

    button_reprendre_sauvegarde = tk.Button(frame_buttons, text="Autres sauvegardes", font=("Arial", 27), command=lambda: (effacer_widget_sans_forget(root), recreer_menu(root), autres_sauvegardes()))
    button_reprendre_sauvegarde.pack(pady=10)

frame_modeles = None

button_selectionne = None


def nouvelle_partie():
    global frame_modeles, frame_partie_finie, difficulte, Sauvegarder
    menu.entryconfig("Sauvegarder", state="disabled")
    # Aide.entryconfig(0, state="disabled")
    ...
    frame_nouvelle_partie = tk.Frame(root, bg=bg_color)
    ...
    button_regeneration = tk.Button(frame_difficulte, text="Regenerer", font=("Arial", 13), command=lambda: changement_difficulte(difficulte, regeneration= False))
    frame_modeles = tk.Frame(frame_nouvelle_partie, bg=bg_color, pady= 10)

        # bf,bn,bd,be = None,None,None,None
    def changement_difficulte(difficulte_a_changer, button_non_affecte = None, regeneration = True):
        global difficulte, button_selectionne
        # global bf,bn,bd,be
        ...
        if regeneration:
            ...
            if button_non_affecte:
                button_selectionne = button_non_affecte
    ...
    button_regeneration.grid(column=1, row=0, padx=3)
    button_facile.grid(column=2, row=0, padx=3)
    button_normal.grid(column=3, row=0, padx=3)
    button_difficile.grid(column=4, row=0, padx=3)
    button_extreme.grid(column=5, row=0, padx=3)
...
def changement_nombre(event):
    ...
    elif event.char in chr_possible and selectionne:
        nombre_choisi = None
    ...
    elif event.char == "0" and selectionne:
        nombre_choisi = None
...
button_aide = None
frame_button_placement = None

def aider():
    global selectionne, correction, ancien_selectionne
    nombre_choisi = correction
    if selectionne:
        sudoku_a_completer["a_completer"][coordonnee[0]][coordonnee[1]][coordonnee[2]][coordonnee[3]] = nombre_choisi
        if valeur == 0:
            selectionne.config(text=str(nombre_choisi), fg="#7FB3FF")
...
def activer_aide(aide):
    global button_aide, frame_button_placement
    if aide.get():
        button_aide = tk.Button(frame_button_placement, text="Aide", font=("Arial", 20), command=lambda: aider())
        button_aide.grid(column=2, row=4, pady=3)
    else:
        button_aide.destroy()
...
def partie_jouable(index, sudoku_liste_choix, sudoku_liste_corrigees):
    global temps, sudoku_choisi, sudoku_choisi_corrige, ind, vie_restantes, label_vie_restantes, label_time, frame_partie_jouable, sudoku_a_completer, Sauvegarder, frame_button_placement
    sudoku_choisi = sudoku_liste_choix
    sudoku_choisi_corrige = sudoku_liste_corrigees
    ind = index

    menu.entryconfig("Sauvegarder", state="normal")
    # Aide.entryconfig(0, state="normal")
...
def fin_partie(gagne):
    ...
    #Nouvelle partie
    button_refaire_une_partie = tk.Button(frame_partie_finie, text="Nouvelle partie", font=("Arial", 27), command=lambda: (effacer_widget_sans_forget(root), recreer_menu(root), nouvelle_partie()))
    button_refaire_une_partie.pack(pady=10)

    #Retour accueil
    button_retour_accueil = tk.Button(frame_partie_finie, text="Retourner à l'accueil", font=("Arial", 27), command=lambda: (effacer_widget_sans_forget(root), creation_menu()))
...
Aide = tk.Menu(menu, tearoff=0)
Aide.add_checkbutton(label="Afficher les indices", variable=aide)
Aide.add_command(label="Ouvrir l'aide en ligne", command=lambda:webbrowser.open("https://www.le-sudoku.fr/le-sudoku/regles"))
menu.add_cascade(label="Aide", menu=Aide)
